...
const DOCK_ITEMS = [
  ...
  { id: "about", icon: Icons.BookOpen, label: "About", href: "#about" },
...
export default function Dock() {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex h-16 items-end gap-4 rounded-2xl border border-white/10 bg-white/5 px-4 pb-3 backdrop-blur-md transition-all duration-300"
    >
      {DOCK_ITEMS.map((item) => (
  ...
  return (
    <Link href={item.href} target={item.external ? "_blank" : undefined}>
      <motion.div
        ref={ref}
        style={{ width }}
        className="aspect-square w-10 rounded-full bg-gray-700/50 border border-black/10 flex items-center justify-center hover:bg-gray-600/80 transition-colors group relative"
      >
